...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <html>
      <head>
        <link rel="stylesheet" type="text/css" href="http://www.memorylifter.com/fileadmin/styles/mlnews_fragment.css" />
      </head>
  ...
  <xsl:template match="item">
    <xsl:if test="position() &lt;= $displayNumber">
      <div class="news-latest-item">
        ...
        <span class="news-latest-date">
          <a href="{link/text()}" title="{title/text()}" target="_blank">
            <xsl:value-of select="user:ConvertDate(pubDate)"/>
          </a>
        </span>
        <h3>
          <a href="{link/text()}" title="{title/text()}" target="_blank">
        ...
        <p class="bodytext">
          <xsl:value-of select="normalize-space(user:CleanTypo3String(description/text()))"/>
        </p>
